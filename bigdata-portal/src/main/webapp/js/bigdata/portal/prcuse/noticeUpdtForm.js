...
let PageObj = (function(){
	...
	PageObj.prototype.setEventHandler = function(){
		...
		$('a.updt').click(function(event){
			...
			//파일 업로드
			$.cs.upload($('[type=file]'), './noticeFileUpload.json', function(fileVos){
				console.log('fileVos : {}', fileVos);
				...
				//데이터 전송
				$.cs.submitAjax('./noticeUpdt.json', p, function(res){
					alert('수정했습니다.');
					
					//
					//$.cs.submitForm($('#uf'), './noticeView.do?nttId=' + $('#uf [name=nttId]').val());
					location.href = './noticeView.do?nttId='+ $('#uf [name=nttId]').val();
				});
