...
<form id="visFrm" name="visFrm" action="<c:url value="/bdp/visual/insert.do" />" method="post" onsubmit="return false;">
...
<h3><i class="ico dataset"></i>분석결과</h3>
...
<div class="wrap_vz">
		...
		<div id="vis_option">
			...
			<dl class="vis" data-section="colortheme" data-chart-use="[1,2,3,4,5,6,7,8]">
				...
				<dd class="btn_radio">
					...
					<ul id="color_palette">
						<li><a class="colorpicker" id="color_choice_btn">색상선택</a><input type="text" id="color_choice" /></li>
					</ul>
				</dd>
			</dl>
			...
			<dl class="vis" data-section="line_style" data-chart-use="[1,3,4]">
				...
				<dd>
					<ul class="stit">
						<li class="tit">유형</li>
						<li>
							<input type="radio" name="chartOption.graph.type" value="line" data-default-checked="true" id="gra_type_line" /><label for="gra_type_line"><span></span>기본</label><br>
							<input type="radio" name="chartOption.graph.type" value="smoothedLine" id="gra_type_smoo" /><label for="gra_type_smoo"><span></span>부드러운</label><br>
			...
			<dl class="vis" data-section="map" data-chart-use="[21]">
				...
				<dd>
					...
					<input type="checkbox" name="chartOption.map.showValue" value="Y" id="map_disp_val"><label for="map_disp_val"><span></span>값표시</label>
				</dd>			
	...
	<div class="tac mt30">
		<input id="visualize_csv_download_btn" type="button" value="분석결과 CSV 다운로드" class="btn grey normal_input"/>
...
$(function() {
		...
		visCreateThumbnail(function(data) {
			...
			$.postJSON($("#visFrm").attr("action"), formdata, function(json) {
				if(json.result == "success") {
					document.location.href = "../visual/list.do";
				} else {
					alert(json.message);
				}
			});
	...
	$("#visualize_csv_download_btn").click(function() {
		var csvRows = [];
