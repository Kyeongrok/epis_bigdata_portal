...
let PageObj = (function(){
	...
	PageObj.prototype.setEventHandler = function(){
		...
		$('a.updt').click(function(){
			$.cs.submitForm($('#df'), './questionUpdtForm.do');
		});
		...
		$('a.delete').click(function(){
			...
			//
			$.cs.submitAjax('./questionDelete.json', p, function(res){
				$('a.list').click();
			});
		});
		...
		$('a.rply').click(function() {
			...
			$.cs.submitAjax('./questionRegRply.json', p, function(res) {
				location.href = './questionView.do?nttId=' + p.nttId;  
			});
		});
	};
});
