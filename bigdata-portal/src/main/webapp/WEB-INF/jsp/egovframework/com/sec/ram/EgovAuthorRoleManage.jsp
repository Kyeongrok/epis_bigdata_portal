...
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/ram/EgovAuthorRoleList.do" method="post">
	<%-- <table width="100%" cellpadding="8" class="table-search" border="0">
		<tr>
			<td width="20%" class="title_left">
				<img src="<c:url value='/images/egovframework/com/cmm/icon/tit_icon.gif' />" width="16" height="16" hspace="3" align="middle" alt="제목아이콘이미지">&nbsp;권한 롤 관리
			</td>
	...
	<h3 class="mt0"><i class="ico ico_arrow_grey"></i>정책 설정정보</h3>
	<div class="searchbox_adm">
		<span class="searcharea">
			<input type="text" name="searchKeyword" value="<c:out value='${authorRoleManageVO.searchKeyword}'/>" onkeypress="press();" readonly="readonly" placeholder="권한코드"/>
			<a href="#LLINK" onclick="javascript:fncSelectAuthorRoleList()"><i class="ico search">조회</i></a>
		</span>
	</div>
	<div class="floatr">
		<a href="#LINK" onclick="javascript:fncSelectAuthorList()" class="btn list_fl">리스트</a>
		<a href="#LINK" onclick="javascript:fncAddAuthorRoleInsert()" class="btn red">등록</a>
	</div>
	
	
	<table class="tbl01 adm_tbl01 column8_role" summary="권한 롤을 관리하는 테이블입니다.롤 ID,롤 명,롤 타입,롤 Sort,롤 설명,등록일자,등록여부의 내용을 담고 있습니다.">
		<thead>
			<tr>
				<th scope="col">
					<!-- <input type="checkbox" name="checkAll" class="chk_basic" onclick="javascript:fncCheckAll()" title="전체선택"> -->
					<input type="checkbox" id="checkAll" name="checkAll" onclick="javascript:fncCheckAll()" title="전체선택">
					<label for="checkAll"><span></span></label>
				</th>
				<th scope="col">설정규칙 ID</th>
				<th scope="col">설정규칙명</th>
				<th scope="col">설정규칙타입</th>
