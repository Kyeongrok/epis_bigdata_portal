...
function fncManageChecked() {
    ...
    var userId = document.listForm.userId;
            ...
            for(var i=0; i<checkField.length; i++) {
                if(checkField[i].checked) {
                    ...
                    if(returnId == "") {
                    	...
                    	returnUserId = returnUserId + ";" + userId[i].value;
    ...
    document.listForm.ids.value = returnUserId;
  ...
  <th width="20%" align="right">
   <table border="0" cellspacing="0" cellpadding="0">
    <tr>
      ...
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_left.gif'/>" width="8" height="20" alt="버튼이미지"></td>
      <td class="btnBackground" nowrap="nowrap">
      	<a href="#LINK" onclick="javascript:fncAddAuthorGroupInsert(); return false;" style="selector-dummy:expression(this.hideFocus=false);">등록</a>
      </td>
      <td><img src="<c:url value='/images/egovframework/com/cmm/btn/bu2_right.gif'/>" width="8" height="20" alt="버튼이미지"></td>
    </tr>
   </table>
	...
	<h3 class="mt0"><i class="ico ico_arrow_grey"></i>회원 권한설정</h3>
	...
	<div class="searchbox_adm">
		...
		<span class="searcharea w70p">
			<input type="text" name="searchKeyword" value="<c:out value='${authorGroupVO.searchKeyword}'/>" onkeypress="press();" title="검색"/>
			<a href="#LINK" onclick="javascript:fncSelectAuthorGroupList('1')"><i class="ico search">조회</i></a>
		</span>
	</div>
	<div class="floatr">
		<a href=" /sec/ram/EgovAuthorList.do" class="btn grey">정책 관리</a>
		<a href="#LINK" onclick="javascript:fncAddAuthorGroupInsert(); return false;" class="btn red">권한 설정</a>
	</div>
	
	<table class="tbl01 adm_tbl01 column5_auth" summary="권한 그룹을 관리하는 테이블입니다.사용자 ID,사용자 명,사용자 유형,권한,등록 여부의 정보를 담고 있습니다.">
		<thead>
			<tr>
				<th scope="col">
					<!-- <input type="checkbox" name="checkAll" class="check2" onclick="javascript:fncCheckAll()" title="전체선택체크박스"> -->
		...
		<c:forEach var="authorGroup" items="${authorGroupList}" varStatus="status">
			<tr>
				<td>
					<!-- <input type="checkbox" name="delYn" class="check2" title="선택" style="display: block;"> -->
					<!-- 체크박스 : 가변적인 id와 label for값이 필요합니다 --> 
					<input type="checkbox" id="checkbox<c:out value='${status.index}'/>" name="delYn" title="선택" >
				...
				<td>
					...
					<input type="hidden" name="userId" value="<c:out value="${authorGroup.userId}"/>"/>
