...
let PageObj = (function(){
	...
	PageObj.prototype.setEventHandler = function(){
		...
		$('a.updt').click(function(){
			$.cs.submitForm($('#df'), './newBigdatasetReqUpdtForm.do');
		});
		...
		$('a.delete').click(function(){
			...
			//
			$.cs.submitAjax('./newBigdatasetReqDelete.json', p, function(res){
				$('a.list').click();
			});
		});
		...
		$('a.rply').click(function() {
			...
			$.cs.submitAjax('./newBigdatasetReqRply.json', p, function(res) {
				location.href = './newBigdatasetReqView.do?nttId=' + p.nttId;  
			});
		});
	};
});
