...
<c:set var="registerFlagName" value="${empty authorManageVO.authorCode ? '신규정책 등록' : '정책 수정'}"/>
...
<form:form commandName="authorManage" method="post" >
	...
	<table class="tbl02 adm_tbl02 column2" summary="권한을 등록하는 테이블입니다.권한 코드,권한 명,설명,등록일자의 정보를 담고 있습니다.">
		<tr>
			<th scope="row">정책 ID <em>*</em></th>
			<td>
				<input name="authorCode" id="authorCode" type="text" value="<c:out value='${authorManage.authorCode}'/>" title="권한코드" />
				<form:errors path="authorCode" />
			</td>
		</tr>
		<tr>
			<th scope="row">정책명 <em>*</em></th>
			<td>
				<input name="authorNm" id="authorNm" type="text" value="<c:out value='${authorManage.authorNm}'/>" title="권한명" />
		...
		<tr>
			<th scope="row">설명</th>
			<td>
				<input name="authorDc" id="authorDc" type="text" value="<c:out value='${authorManage.authorDc}'/>" title="설명" />
			</td>
		</tr>
	</table>
