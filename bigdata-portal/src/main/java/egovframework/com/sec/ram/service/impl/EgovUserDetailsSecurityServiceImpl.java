...
public class EgovUserDetailsSecurityServiceImpl extends EgovAbstractServiceImpl implements EgovUserDetailsService {
	...
	public Object getAuthenticatedUser() {
		...
		// Ambari Session Value를 가지고 있을 수 있도록 설정
		...
		if (EgovUserDetailsHelper.isAuthenticated()) {
			...
			if(user != null) {
				LoginVO userSessionInfo = (LoginVO) RequestContextHolder.getRequestAttributes().getAttribute("loginVO", RequestAttributes.SCOPE_SESSION);
				if (userSessionInfo == null) {
					return null;
				}
				
				System.out.println(userSessionInfo.toString());
				((LoginVO) user).setAmbariSessionValue(userSessionInfo.getAmbariSessionValue());
			}		
