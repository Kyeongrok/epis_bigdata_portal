...
public class EgovLoginPolicyFilter implements Filter {
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain) throws IOException, ServletException {
        ...
        if (id == null || userSe == null) {
            ((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/bdp/auth/login.do");
        }
        ...
        try {
            ...
            ((HttpServletResponse) response).sendRedirect(httpRequest.getContextPath() + "/bdp/auth/login.do?login_error=1");
            return ;
        }
    }
