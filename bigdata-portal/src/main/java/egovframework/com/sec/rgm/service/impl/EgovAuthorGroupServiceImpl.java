...
public class EgovAuthorGroupServiceImpl  extends EgovAbstractServiceImpl implements EgovAuthorGroupService {
	...
	/**
	 * 그룹 리스트를 불러와 회원의 기본 권한 설정 
	 */
	public void insertAuthorGroup(AuthorGroupVO authorGroupVO, Object manageVO) throws Exception {
		
		AuthorGroup authorGroup = authorGroupDAO.selectAuthorGroupList(authorGroupVO).get(0);		
		authorGroup.setAuthorCode(authorGroupVO.getAuthorCode());
		
		if(manageVO instanceof MberManageVO) {
			String uniqId = ((MberManageVO) manageVO).getUniqId();
			authorGroup.setUniqId(uniqId);
		} else if(manageVO instanceof EntrprsManageVO) {
			String uniqId = ((EntrprsManageVO) manageVO).getUniqId();
			authorGroup.setUniqId(uniqId);
		} else if (manageVO instanceof UserManageVO) {
			String uniqId = ((UserManageVO) manageVO).getUniqId();
			authorGroup.setUniqId(uniqId);
		}
