...
<form:form name="listForm" action="${pageContext.request.contextPath}/sec/ram/EgovAuthorList.do" method="post">
	<h3 class="mt0"><i class="ico ico_arrow_grey"></i>정책 관리</h3>
	<div class="searchbox_adm">
		<span class="searcharea">
			<input type="text" name="searchKeyword" value="<c:out value='${authorManageVO.searchKeyword}'/>" onkeypress="press();" placeholder="권한 명 검색"/>
			<a href="#LINK" onclick="javascript:fncSelectAuthorList('1')"><i class="ico search">조회</i></a>
		</span>
	</div>
	<div class="floatr">
		<a href="#LINK" onclick="javascript:fncAddAuthorInsert()" class="btn red">등록</a>
		<a href="#LINK" onclick="javascript:fncAuthorDeleteList()" class="btn grey">삭제</a>
	</div>
	
	<table class="tbl01 adm_tbl01 column6" summary="정책관리에  관한 테이블입니다.정책ID,정책 명,설명, 등록일자, 정책설정 정보의 내용을 담고 있습니다.">
		<thead>
			<tr>
				<th>
					<!-- <input type="checkbox" name="checkAll" class="chk_basic" onclick="javascript:fncCheckAll()" title="전체선택체크박스"> -->
		...
		<c:forEach var="author" items="${authorList}" varStatus="status">
			<tr>
				...
				<td class="adm_tbl_ico">
					<a href="#LINK" onclick="javascript:fncSelectAuthorRole('<c:out value="${author.authorCode}"/>')" class="btn lightgrey sm w100p bdbox">
						<%-- <img src="<c:url value='/images/egovframework/com/cmm/icon/search.gif'/>" width="15" height="15" align="middle" alt="롤 정보"> --%>
						<i class="ico search">조회</i>
					</a>
				</td>
			</tr>
		</c:forEach>
