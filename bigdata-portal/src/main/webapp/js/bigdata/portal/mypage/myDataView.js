/**
 * 나의 데이터 조회 
 */
let PageObj = (function(){
	
	/**
	 * 초기화
	 */
	PageObj.prototype.init = function(){
		pageObj.setEventHandler();
	}; 
	
	/**
	 * 이벤트 등록
	 */
	PageObj.prototype.setEventHandler = function() {
		//데이터업로드
		$('a.upload').click(function() {
			$.post("./kafkaUpload.do", $("#df").serialize(), function(json, result, aa) {
				console.log(aa);
				if(result == "success") {
					alert('데이터 업로드를 완료했습니다.');
				} else {
					alert('데이터 업로드를 실패했습니다.');
				}
			});
		});
		
		//수정
		$('a.updt').click(function(){
			$.cs.submitForm($('#df'), './myDataUpdtForm.do');
		});
		
		//삭제
		$('a.delete').click(function(){
			if(!confirm('삭제하시겠습니까?')){
				return;
			}
			
			//
			let p = {};
			p.nttId = $('#df [name=nttId]').val();
			
			//
			$.cs.submitAjax('./myDataDelete.json', p, function(res){
				$('a.list').click();
			});
		});
		
		//목록
		$('a.list').click(function(){
			
			location.href = './myDataList.do';
		});
	};
});

//
let pageObj = new PageObj();
$(document).ready(function(){
	pageObj.init();
});