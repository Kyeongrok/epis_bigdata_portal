...
let PageObj = (function(){
	...
	PageObj.prototype.setEventHandler = function(){
		...
		$('a.updt').click(function(event){
			...
			//파일 업로드
			$.cs.upload($('[type=file]'), './analysisReqFileUpload.json', function(fileVos){
				...
				//데이터 전송
				$.cs.submitAjax('./analysisReqUpdt.json', p, function(res){
					alert('수정했습니다.');
					
					location.href = './analysisReqView.do?nttId='+ $('#uf [name=nttId]').val();
				});
