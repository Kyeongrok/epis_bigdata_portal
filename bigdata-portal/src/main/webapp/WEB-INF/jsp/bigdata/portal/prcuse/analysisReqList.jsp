...
<script type="text/javascript" src="<c:url value="/js/bigdata/portal/prcuse/anlysisReqList.js" />"></script>
...
<div class="tablearea">
	...
	<table class="tbl01 " summary="분석대행신청 게시판입니다.">
		...
		<c:forEach var="row" items="${datas}" varStatus="status">
			<tr>
				<td>${row.nttNo}</td>
				<td><a href="<c:url value='./analysisReqView.do'/>?nttId=${row.nttId}">${row.nttSj}</a></td>
				<td>${row.ntcrNm}</td>
				<td>${row.rdcnt}</td>
...
<c:if test="${isLogin == true}">
	<a href="<c:url value='./registAnalysisReqForm.do'/>" class="floatr btn red">등록</a>
</c:if>

<script type="text/javascript">
function fn_link_page(pageNo) {
	location.href = "<c:url value='/bdp/prcuse/analysisReqList.do'/>?pageIndex="+pageNo;
}
