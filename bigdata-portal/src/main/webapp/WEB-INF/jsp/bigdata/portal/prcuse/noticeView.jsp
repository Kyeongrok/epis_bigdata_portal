...
<script type="text/javascript" src="<c:url value="/js/bigdata/portal/prcuse/noticeView.js" />"></script>


<div class="wrap_admin">

	<h3 class="mt0"><i class="ico ico_arrow_grey"></i>공지사항 조회</h3>
			...
			<td colspan="3">
				<c:if test="${!empty files }">
					<c:forEach items="${files }" var="row">
						<div>
							<%-- <img src="./noticeDwld.do?atchFileId=${row.atchFileId }&fileNo=${row.fileNo }" width="800" height="600" alt="${row.originFileName }"/> --%>
						</div>
					</c:forEach>
				</c:if>
				...
				<c:if test="${!empty files }">
					<c:forEach items="${files}" var="row">
						<div><a href="/cmm/fms/FileDown.do?atchFileId=${row.atchFileId}&fileSn=${row.fileNo}">${row.originFileName } (${row.fileSize }바이트)</a></div>
